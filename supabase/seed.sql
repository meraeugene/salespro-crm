...
--   rep2@salespro.test      role: sales_representative
--   rep3@salespro.test      role: sales_representative
...
values
  ...
  ('60000000-0000-4000-8000-000000000003', 'Globax wants a phased deployment with operations and sales going live first, then regional managers one month later.', 'deal', '40000000-0000-4000-8000-000000000003', (select id from public.profiles where email = 'manager@salespro.test'), '2026-05-22 13:10+08'),
...
values
  ('70000000-0000-4000-8000-000000000001', 'Deal moved to Proposal Sent', 'deal', '40000000-0000-4000-8000-000000000001', jsonb_build_object('stage', 'Proposal Sent', 'title', 'Stellar Enterprise CRM', 'company', 'Stellar Dynamics', 'value', 142000, 'forecast_category', 'Best Case', 'review_status', 'Pending Review'), (select id from public.profiles where email = 'manager@salespro.test'), '2026-05-18 15:00+08'),
  ('70000000-0000-4000-8000-000000000002', 'Deal approved', 'deal', '40000000-0000-4000-8000-000000000003', jsonb_build_object('title', 'Globax Deployment', 'company', 'Globax Corporation', 'review_status', 'Approved', 'approved_by', 'Maya Chen'), (select id from public.profiles where email = 'manager@salespro.test'), '2026-05-23 09:00+08'),
  ('70000000-0000-4000-8000-000000000003', 'Deal moved to Won', 'deal', '40000000-0000-4000-8000-000000000003', jsonb_build_object('stage', 'Won', 'title', 'Globax Deployment', 'company', 'Globax Corporation', 'value', 228000, 'probability', 100), (select id from public.profiles where email = 'manager@salespro.test'), '2026-05-23 09:05+08'),
  ('70000000-0000-4000-8000-000000000004', 'Lead assigned', 'lead', '20000000-0000-4000-8000-000000000002', jsonb_build_object('title', 'Benjamin Ross', 'company', 'Acme Inc.', 'assigned_to', 'Sophia Martinez'), (select id from public.profiles where email = 'manager@salespro.test'), '2026-05-22 11:15+08'),
  ('70000000-0000-4000-8000-000000000005', 'Task completed', 'task', '50000000-0000-4000-8000-000000000003', jsonb_build_object('title', 'Prepare Globax stakeholder map', 'status', 'Done'), (select id from public.profiles where email = 'manager@salespro.test'), '2026-05-21 17:20+08'),
  ('70000000-0000-4000-8000-000000000006', 'Call logged', 'deal', '40000000-0000-4000-8000-000000000007', jsonb_build_object('activity_type', 'Call', 'subject', 'BrightWave compliance call', 'body', 'Reviewed audit trail and approval workflow requirements.', 'outcome', 'Requested data flow summary', 'scheduled_at', null), (select id from public.profiles where email = 'manager@salespro.test'), '2026-05-23 14:30+08')
on conflict (id) do update set
