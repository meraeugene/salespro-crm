...
export async function GET() {
  ...
  const { data, error: dbError } = await supabase.from("contacts").select("*").order("created_at", { ascending: false });
...
export async function PATCH(request: Request) {
  ...
  const { data, error: dbError } = await supabase.from("contacts").update(updates).eq("id", id).select().single();
  if (dbError) return handleError(dbError);
  return NextResponse.json(data);
}
